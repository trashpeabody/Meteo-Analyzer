const prepareMetar = (message) => {
  const metarArray = message.toUpperCase().split(' ')
  return metarArray.map(element => element.replace(/\r?\n/g, ' ').trim()).filter(element => element.toString === '')
}

function functionPrevia (message) {
  const metarWind = 'NOWIND'					// declaración de variables de viento, visibilidad, rvr, tiempo significativo, nubosidad y visibilidad vertical
  const metarVisibility = 'NOVIS'

  const metarArray = prepareMetar(message)

  // base de datos de aeropuertos horarios o semihorarios
  const semiHorarios = ['LETU', 'LECO', 'LEAG', 'LEAL', 'LEAM', 'LESU', 'LEAS', 'LEBL', 'LEBR', 'LEBB', 'LEBG', 'LECH', 'LEBA', 'GCFV', 'LEGE', 'GCLP', 'LEGR', 'GCHI', 'LEHC', 'LEIB', 'LEJR', 'GCGM', 'GCLA', 'GCRR', 'LEDA', 'LEMD', 'LEVS', 'LEMG', 'GEML', 'LEMH', 'LPEA', 'LEPP', 'LERS', 'LELL', 'LESA', 'LESO', 'LEXJ', 'LEST', 'LEZL', 'GCXO', 'GCTS', 'LETL', 'LEVC', 'LEVD', 'LEVX', 'LEVT', 'LEZG']
  const horarios = ['LEAB', 'LEBZ', 'LERT', 'LEAO', 'LEGA', 'LELN', 'LELO', 'LECV', 'LEGT', 'LETO', 'LERI', 'LELC', 'LEMO', 'LEEC', 'LEBT']

  if (metarArray[0] === undefined) {
    return null
  }
