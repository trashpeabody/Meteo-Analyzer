...
export interface WindVarResult {
  matches: boolean,
  result: Result
}

export const checkWindVar = (windVar: string, avgWindDirection: string, avgWindSpeed: string): WindVarResult => {
  const varWindRegExp = /^(\d{3})(V)(\d{3})(=)?$/
  let result: Result

  const checkPrevCondition = (): Result => {
